                ...
                if(productList.user_image !== null){
                    ...
                    <div id="" class="align-self-center product_detail_up_text">
                        ...
                        <div id="" class="product_detail_up_button">
                            <!-- 綁定驗證是否已登入 -->
                            <button id="changeBTN" class="btn btn-primary btn-lg">我要交換</button>
                        </div>
                    </div>
                    <div id=""  class="text-center product_detail_userimage">
                        <img class="rounded-circle" src="/image/member/upload/headshot/${productList.user_image}"></img>
                            ...
                            $.each(productList,function(index,item){
                                ...
                                <div id="" class="BWC_content">
                                    <div class="BWC_content_inside ms-3">
                                        ...
                                        <div id="" class="dill_date">申請時間 | ${productList[index].dill_date}</div>
                                    </div>
                                </div>
                                </div>                                     
                            `);
                            ...
                            if(e.length !== 0){
                                $.each(e,function(index,item){
                                    ...
                                    if(e[index].reply !== null){
                                        ...
                                        <div>回覆 : ${e[index].reply} <span class="dill_date"> 於( ${e[index].reply_date} )</span></div>
                                        </div>
                                        `)
                                    }else{
                                        $('.showQA').append(`
...
var closedialogalert = () => {
    document.querySelector('#custom-alert').close();   
}
...
function showAlert2(message) {
  // Display the custom alert dialog box
  $("#custom-alert2").removeClass("hidden");
  $("#custom-alert-message2").text(message);
  document.querySelector('#custom-alert2').showModal();
  // Hide the custom alert dialog box after 2 seconds
  setTimeout(function() {
    $("#custom-alert2").addClass("hidden");
    document.querySelector('#custom-alert2').close();   
  }, 1200);
}


function showConfirm(message) {
    document.querySelector('#custom-confirm').showModal();
  ...
  $("#custom-confirm-okay").on("click", function() {
    $('#changeForm').submit(function(event) {
        event.preventDefault();
      });
    document.querySelector('#custom-confirm').close(); 
...
$("#custom-confirm-cancel").click(function() {
    $('#changeForm').submit(function(event) {
        event.preventDefault();
      });
    document.querySelector('#custom-confirm').close(); 
...
function myconfirm() {
    ...
    if (checkboxS !== true) {
          $('#changeForm').submit(function(event) {
            ...
            showAlert2('請選取至少一個商品喔!');
    } else {
          // Example usage
  $('#changeForm').submit(function(event) {
    event.preventDefault();
  });
      showConfirm("確定要申請交換此物品?");
    }
  }
