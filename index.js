...
app.get('/product/Search/Search', function (req, res) {
    var sql = "SELECT product_id, product_name, product_image , user_name, city , DATE_FORMAT(lunch_date, '%Y/%m/%d %H:%i')lunch_date FROM product_page.product WHERE product_name LIKE ? ORDER BY lunch_date DESC";
...
app.get('/product/citySearch/citySearch', function (req, res) {
    ...
    if (length !== 0) {
        var data = array
        sql = "SELECT product_id, product_name, product_image , user_name, city , DATE_FORMAT(lunch_date, '%Y/%m/%d %H:%i')lunch_date FROM product_page.product WHERE city IN (?) ORDER BY lunch_date DESC";
        // console.log(data);
    }
...
app.get('/product/:product_detail/detail/product_detail', function (req, res) {
    var sql = "SELECT product_id, product_name, product_image, user_name, city, user_image, product_detail, DATE_FORMAT(lunch_date, '%Y/%m/%d %H:%i') as lunch_date, method FROM product_page.product WHERE product_id = ?";
    var data = [req.params.product_detail];
    
    conn.query(sql, data, async function (err, results, fields) {
        if (err) {
            res.send('select發生錯誤', err);
        } else {
            var sql1 = "SELECT headshot FROM membercenter.personal WHERE username = ?";
            await conn.query(sql1, [results[0].user_name], function (err, results1, fields) {
                if (err) {
                    res.send('select發生錯誤', err);
                } else {
                    results[0].user_image = results1[0].headshot; // 更換圖片位址承會員中心的上傳圖片
                    console.log(results[0].user_image);
                    res.json(results[0]); // 傳回更改後的第一個選擇結果
                }
            });
        }
    });
...
app.post('/product/WCPinfo', function (req, res) {
    var sql = "SELECT BWC_user_name,BWC_product_id,BWC_product_name,BWC_product_image,BWC_city,BWC_lunch_date,WC_product_id,BWC_method , DATE_FORMAT(dill_date, '%Y-%m-%d %H:%i:%s')dill_date FROM product_page.bwc WHERE WC_product_id = ?;";
...
app.get('/product/member/QA', function (req, res) {
    var sql = "SELECT `id`, `product_id`, `product_name`, `member_id`, `username`, `content`, `reply`,DATE_FORMAT(Question_date, '%Y/%m/%d %H:%i')Question_date ,DATE_FORMAT(reply_date, '%Y/%m/%d %H:%i')reply_date FROM product_page.qa WHERE product_id = ?;"
